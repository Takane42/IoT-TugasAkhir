...
const nim = "G.231.22.0154";
const topic = `iot/${nim}`;
const topicPub = `iot/${nim}/sub`;
...
function onMessageArrived(message) {
    ...
    // Check if the message is a LED control command (not JSON)
    if (message.payloadString.startsWith('LED')) {
        console.log('Received LED control command:', message.payloadString);
        // You could handle LED state synchronization here if needed
        return;
    }
    
    // Otherwise, treat it as JSON data with sensor readings
    try {
        const data = JSON.parse(message.payloadString);
        const suhu = parseFloat(data.suhu);
        const kelembaban = parseFloat(data.kelembapan);
...
function toggleLamp(pin) {
    ...
    const newState = currentState; // We don't need to toggle since the checkbox was already updated when clicked
